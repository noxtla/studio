...
import { useToast } from "@/hooks/use-toast"; // Todavía útil para mensajes de quiz completado
...
export default function SafetyModulesPage() {
  ...
  const handleElaborate = useCallback(async () => {
    ...
    setIsElaborating(false);
  }, [currentCard, toast]); // CAMBIADO: Añadimos 'toast' aquí para resolver la advertencia/error del linter.
      ...
      <Card className="w-full max-w-2xl text-center shadow-lg rounded-quiz-lg p-6 flex flex-col gap-5">
          ...
          {sectionsData.map((section, index) => (
            <Button
              ...
              onClick={() => {
                ...
                // Volver al mensaje de bienvenida de la sección al cambiar
                displayMessage(sectionsData[index].name, false);
              }}
              // CAMBIADO: Usando 'bg-quiz-action' y sus variantes de hover
              className={cn(
                "section-nav-button px-5 py-2 rounded-quiz-xs font-semibold",
                index === currentSectionIndex ? "bg-quiz-action text-white hover:bg-quiz-action-darker" : "bg-gray-200 text-gray-700 hover:bg-gray-300",
                "transition-all duration-200 ease-in-out"
              )}
              variant="ghost" // Usamos variant ghost para tener un control total del bg y text
            >
              {section.name}
            </Button>
          ))}
        ...
        {!quizStarted && (
          ...
          <div className="bg-quiz-message-blue text-quiz-message-blue-text p-5 rounded-quiz-md font-medium flex flex-col gap-4">
            ...
            {currentSectionIndex < sectionsData.length - 1 && activeDeck.every(card => card.completedOnce) && (
              <Button
                onClick={() => {
                  setCurrentSectionIndex(prev => prev + 1);
                  displayMessage(sectionsData[currentSectionIndex + 1].name, false);
                }}
                // CAMBIADO: Usando 'bg-quiz-action' y sus variantes de hover
                className="bg-quiz-action hover:bg-quiz-action-darker py-3 rounded-quiz-sm font-semibold shadow-md transition-all duration-200 hover:translate-y-[-2px] hover:shadow-lg"
              >
